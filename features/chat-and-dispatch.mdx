...
Supported types include images (PNG, JPG), PDF, DOCX, and XLSX. Images show as thumbnails (click in chat to open a full-screen lightbox); other files show as removable chips.
...
During a run the agent sees its Soul, recent conversation, unread inbox messages, references, and team context, then works in the working directory and reports back. Its tool activity streams live into the chat and the Activity tray.

## Agent-to-agent messages

Coordination traffic is kept out of your chat. Read it in:

- **Dispatch** tab: the whole team's message feed.
- **Agent Comms** tab (per agent): messages between Atlas and that agent, with filters for **All**, **@user** (things flagged for you), or a specific teammate.
